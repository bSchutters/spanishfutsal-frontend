    ...
    <div ref={conteneur} className="relative aspect-video w-full bg-black">
      {erreur && (
        <div className="absolute inset-0 z-20 flex flex-col items-center justify-center gap-3 bg-spanish-bg-dark px-6 text-center">
          ...
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="rounded-md bg-spanish-accent-2 px-4 py-2 text-sm font-bold uppercase text-spanish-bg"
          >
      ...
      {pret && !erreur && muet && (
        <button
          type="button"
          onClick={basculerSon}
          className="absolute inset-x-0 top-4 z-20 mx-auto flex w-fit cursor-pointer items-center gap-2 rounded-full bg-spanish-accent-2 px-4 py-2 text-sm font-bold uppercase text-spanish-bg transition-[scale,background-color] duration-200 ease-[var(--ease-out-strong)] hover:bg-spanish-accent-2-dark active:scale-95 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-white"
        >
      ...
      <div className="absolute inset-x-0 bottom-0 z-10 flex items-center gap-2 bg-gradient-to-t from-black via-black/80 to-transparent px-3 pb-3 pt-24 sm:gap-3 sm:px-4">
        <button
          ...
          className="flex size-11 cursor-pointer items-center justify-center rounded-full bg-spanish-accent-2 text-spanish-bg transition-[scale,background-color] duration-200 ease-[var(--ease-out-strong)] hover:bg-spanish-accent-2-dark active:scale-95 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-white"
        >
          {enLecture ? (
            <Pause className="size-5 fill-spanish-bg" aria-hidden />
          ) : (
            <Play
              className="size-5 translate-x-0.5 fill-spanish-bg"
              aria-hidden
            />
          )}
        </button>
