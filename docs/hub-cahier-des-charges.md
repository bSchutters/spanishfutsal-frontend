...
| `/hub/membres` | administrateurs | accès au Hub, modules et flux de chaque compte (ajouté le 17/09/2026) |
| `/hub/joueurs/effectif` | module joueurs | l'effectif, fiches des joueurs et du staff, numéros de feuille de match (ajouté le 16/09/2026, élargi le 17/09) |
| `/api/hub/photos` | module joueurs en édition | dépôt d'une photo de joueur vers les Médias du site (17/09/2026) |
| `/hub/joueurs/stats` | module joueurs | matchs de la saison et état de saisie des stats (ajouté le 16/09/2026) |
| `/hub/joueurs/stats/[id]` | module joueurs | feuille de stats d'un match (ajouté le 16/09/2026) |
...
| `visuels` | upload multiple | collection `hub-media` dédiée, originaux gardés tels quels |
...
| `votes` | relation multiple → Users | un vote pour par utilisateur, indicatif, sans effet sur le statut |
| `votes_against` | relation multiple → Users | un vote contre par utilisateur ; une personne est pour ou contre, jamais les deux |
...
| Idees (16/09/2026) | Tableau a trois colonnes, les plus recentes en haut, sans tri par votes (l equipe compte six ou sept personnes). Pas de glisser-deposer : Retenir, Ecarter et Rouvrir sont des boutons sur la carte et dans le ticket, a la demande de Bryan. Le vote se presente en sondage, pour ou contre (champ `votes_against`, meme table de relations, sans migration), l un excluant l autre, « 2 pour · 1 contre · sur 7 » avec les noms. Un bouton « Inspi » sur la carte ouvre le lien d inspiration. Une pastille sur l entree Idees du menu compte les nouvelles idees sans son vote. Onglets par colonne sur telephone, bouton flottant pour noter vite. Le ticket s ouvre en panneau lateral comme un evenement, l adresse `/hub/idees/[id]` y renvoie. Le vote est ouvert des la lecture et s ecrit en systeme apres verification que l idee est lisible. « Planifier » ouvre le formulaire d evenement pre-rempli (type Post, titre, description, reseaux, formats, match lie) ; a l enregistrement l idee passe en Retenue avec le lien du post. Une idee vise plusieurs formats, comme un post (migration `ideas_formats_multiples`). Pas d apercu du lien d inspiration en V1, le lien s affiche tel quel |
| Lot 5 valide (16/09/2026) | Apres les retours de Bryan : boutons de statut en pastel, vote pour ou contre en sondage avec les prenoms, pastille des idees a voter dans le menu, bouton Inspi, entree Calendrier repliable. Le prenom seul s affiche partout dans le Hub |
| PWA et push (16/09/2026) | Manifeste propre au Hub servi par `/hub/manifest.webmanifest`, icones rendues du blason par `/hub/icone/[taille]` (variante `?fond=1` pour les icones masquees), service worker `public/hub-sw.js` limite a `/hub`, sans cache. Le job des rappels tourne derriere `/api/hub/rappels` (CRON_SECRET, toutes les cinq minutes), reserve chaque rappel dans le journal avant d envoyer. Cles VAPID dans `.env` (`VAPID_PUBLIC_KEY`, `VAPID_PRIVATE_KEY`, `VAPID_SUBJECT`). Le push exige https : impossible a essayer depuis un telephone sur la copie http du reseau local, il faut un deploiement ou un tunnel https |
| Visuels des posts (16/09/2026) | Collection `hub-media` a part des medias du site : l original est garde tel quel, jamais converti ni redimensionne, seule une vignette WebP de 480 px est derivee pour les grilles. Images et videos. Depot depuis le formulaire par l API Payload avec la session, en plus du lien. Dans le panneau, une galerie avec « Enregistrer » sur telephone (feuille de partage du systeme, donc galerie) et « Telecharger » ailleurs. Lecture reservee au module calendrier ; sur Vercel Blob, l adresse du fichier reste publique pour qui la connait, ce qui convient a des visuels destines aux reseaux. Deux migrations (`visuels_hub`, `visuels_hub_ancien_lien`) |
...
| Module Joueurs (16/09/2026) | Deuxième module du registre, `players`, demandé par Bryan une fois le Hub en production, hors V1 du cahier. Deux pages sous l entrée Joueurs, Effectif puis Stats, sans page intermédiaire. Lecture : consulter ; édition : saisir. Les droits se donnent par module, comme le calendrier |
| Numéros de feuille de match (16/09/2026) | Les numéros du site (`numero`) ne sont pas ceux des feuilles de match : le club n a que treize maillots pour tout l effectif, un numéro se partage entre plusieurs joueurs. Deux champs sur la fiche joueur, `numero_feuille_1` et `numero_feuille_2` (migrations `joueurs_second_numero` puis `numeros_feuille_de_match`, la première ayant d abord ajouté un « second numéro » abandonné dans la foulée). La page Numéros liste gardiens puis joueurs de champ par nom avec leurs deux numéros, saisie en ligne, case vidée = numéro effacé, et une vue par numéro qui dit qui peut porter quel maillot. Affichage par défaut, grands chiffres ; le bouton Modifier ouvre la saisie. Les maillots vont du 2 au 14 pour les joueurs de champ, treize numéros ; les gardiens ont le 1 et le 21. Seule plage acceptée, par poste (17/09/2026). Un numéro se donne à deux joueurs au plus, l un en principal, l autre en secondaire (rotation voulue par Bryan), jamais à trois, et un joueur ne le prend pas deux fois : vérifié dans le navigateur et sur le serveur. La vue par numéro liste les treize maillots avec leurs porteurs et les places libres, et signale en rouge un numéro saisi hors de la plage. Le staff n y figure pas, le numéro du site n est jamais touché |
| Feuille de stats (16/09/2026) | Les statistiques restent dans les deux tableaux de la collection Matchs (`field_players_stats`, `goalkeeper_stats`) que le site public lit : le Hub en est un éditeur, pas un nouveau stockage. Une feuille par match de la saison active : on coche qui a joué, puis buts, assists, jaunes (2 au plus), rouge (1 au plus) et clean sheet du gardien, au bouton. Un joueur coché sans rien compte un match joué. Le total des buts est comparé au score du club, sans bloquer. Les matchs sont rangés À saisir (score sans feuille), Saisie, À venir (sans score). Le match d essai et les matchs sans date sont exclus, à la demande de Bryan. L écriture se fait en système après vérification du droit d édition du module, limitée aux deux tableaux, et le crochet de la collection fait suivre le calendrier et le cache du site |
| Effectif (17/09/2026) | La page Numéros devient Effectif, à la demande de Bryan : toute la collection Joueurs se gère depuis le Hub. Trois catégories, Gardiens, Joueurs, Staff, les inactifs estompés en fin de catégorie, même disposition en trois colonnes (les deux numéros de feuille, puis la personne avec sa photo). Pas de bouton Modifier : un clic sur une ligne ouvre la fiche en panneau latéral, un bouton Ajouter crée une fiche. La fiche porte prénom, nom, poste, photo, date de naissance, numéro du site, numéros de feuille, capitaine, actif. La photo part dans les Médias du site par `/api/hub/photos` (droit du module en édition, écriture en système, conversion WebP comme toute image du site). Rien ne se supprime : une fiche sort de l effectif en décochant Actif. Le bloc Maillots reste sous la liste |
| Un seul numéro (17/09/2026) | Retour en arrière demandé par Bryan : les deux numéros de feuille de match et les règles de maillots (2 à 14, 1 et 21, deux porteurs par numéro) sont retirés, et le bloc Maillots avec. Le numéro de la collection Joueurs, celui du site, est le seul, affiché dans l Effectif comme sur la feuille de stats. Le staff n en a pas. Migration `numero_unique` : les seize numéros de feuille déjà saisis sont recopiés dans ce champ avant que les colonnes ne disparaissent, l ancien contenu étant restitué par le `down` |
| Module Direct (17/09/2026) | Troisième module du registre, `live`, demandé par Bryan une fois le direct en production. Une seule entrée, Direct, qui liste les diffusions passées et ouvre le détail d une soirée. Lecture seule : rien ne s y saisit, les chiffres viennent des collections `live-reports` et `live-audience` remplies par le site. Migration `module_direct` pour ajouter la valeur au type énuméré des modules |
| Rapport de diffusion (17/09/2026) | Le rapport de fin de match n est plus pousse vers Discord : Bryan a supprime le salon, il consultera le Hub quand il voudra savoir. Tout l envoi est retire, champ des Paramètres, commande d essai et case « rapport envoye » comprises (migration `sans_discord`). La page du Hub devient l endroit ou les chiffres se lisent, avec la comparaison d un match a l autre que Discord ne pouvait pas donner. Le rapport du match d essai est ecrit comme les autres mais n apparait pas dans la liste |
| Droits depuis le Hub (17/09/2026) | Ouvrir un module demandait d aller dans chaque fiche de l admin Payload. Une page `/hub/membres`, réservée aux administrateurs, règle l accès au Hub, le niveau de chaque module et les flux autorisés, compte par compte. Elle se rejoint par le menu du compte, et par le profil sur téléphone. Elle ne touche ni au rôle, ni au mot de passe, ni aux notifications que chacun choisit : ceux-là restent dans Payload ou dans le profil. La liste des modules vient du registre, donc un module ajouté au code y apparaît sans rien d autre à faire. Lecture et écriture avec les droits de la personne, `overrideAccess: false`, donc les règles de Users s appliquent une seconde fois |
